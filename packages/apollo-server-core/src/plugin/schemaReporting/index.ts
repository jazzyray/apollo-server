...
export function ApolloServerPluginSchemaReporting(
  ...
  return {
    ...
    async serverWillStart({ apollo, schema, logger }) {
      ...
      logger.info('Starting schema reporter...');
      if (overrideReportedSchema !== undefined) {
        logger.info('Schema to report has been overridden');
      }
      if (initialDelayMaxMs !== undefined) {
        logger.info(
          `Schema reporting max initial delay override: ${initialDelayMaxMs} ms`,
        );
      }
      if (endpointUrl !== undefined) {
        logger.info(`Schema reporting URL override: ${endpointUrl}`);
      }
