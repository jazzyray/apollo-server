...
export function ApolloServerPluginUsageReporting<TContext>(
  ...
  return {
    ...
    serverWillStart({
      ...
      // Needs to be an arrow function to be confident that apiKey is defined.
      const sendReport = async (executableSchemaId: string): Promise<void> => {
        ...
        if (options.debugPrintReports) {
          ...
          logger.warn(`Engine report: status ${response.status}`);
        }
      };
      ...
      requestDidStartHandler = ({
        ...
        function didEnd(
          ...
          async function addTrace(): Promise<void> {
            ...
            // Ensure that the caller of addTrace (which does not await it)
            // is not blocked.
...
function makeHTTPRequestHeaders(
  http: Trace.IHTTP,
  headers: Headers,
